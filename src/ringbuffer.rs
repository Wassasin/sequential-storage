...
use crate::{
    cache::{NoCache, PrivateCacheImpl},
    item::{Item, ItemHeader, ItemHeaderIter},
};
...
pub struct RingbufferConfig<S> {
    flash_range: Range<u32>,
    _phantom: PhantomData<S>,
}

impl<S: NorFlash> RingbufferConfig<S> {
...
/// A strict fifo queue storage or a ringbuffer
///
...
/// # });
/// ```
pub struct RingbufferStorage<S: NorFlash> {
    inner: GenericStorage<S, NoCache>,
    state: Option<State>,
}

#[derive(Clone)]
struct State {
    /// Page that points to last data, if any.
    end_page: usize,
    /// Page that points to first data, if any.
    start_page: usize,
}

impl<S: NorFlash> RingbufferStorage<S> {
    ...
    pub const fn new(storage: S, config: RingbufferConfig<S>) -> Self {
        Self {
    ...
    async fn ensure_state(&mut self) -> Result<State, Error<S::Error>> {
        Ok(if let Some(state) = self.state.as_ref() {
            state.clone()
        } else {
            self.state = Some(self.compute_state().await?);
            // Safety: we just set it.
            unsafe { self.state.as_ref().unwrap_unchecked().clone() }
        })
    }

    async fn compute_state(&mut self) -> Result<State, Error<S::Error>> {
        // Find first Closed or PartialOpen.
        ...
        let Some(start_page) = start_page else {
            // If no Closed pages have been found, all must be Open, thus we can start anywhere.
            return Ok(State {
                start_page: 0,
                end_page: 0,
            });
        ...
        // We have found our first page, but where does it end?

        for page_index in self.inner.get_pages(start_page) {
            match self.inner.get_page_state(page_index).await? {
                PageState::PartialOpen => {
                    return Ok(State {
                        end_page: page_index,
                ...
                PageState::Open => {
                    // Invalid state, first an Closed state, followed by an Open state.
                    return Err(Error::Corrupted {
                        #[cfg(feature = "_test")]
                        backtrace: std::backtrace::Backtrace::capture(),
                    });
        ...
        // Only Closed states found, also an invalid state!
        return Err(Error::Corrupted {
            #[cfg(feature = "_test")]
            backtrace: std::backtrace::Backtrace::capture(),
        });
...
mod tests {
    ...
    async fn peek_and_overwrite_old_data() {
        let mut storage = QueueStorage::new(
            ...
            const { RingbufferConfig::new(0x00..0x40) },
            NoCache::new(),
        );
    ...
    async fn push_pop() {
        let mut storage = QueueStorage::new(
            ...
            const { RingbufferConfig::new(0x000..0x1000) },
            NoCache::new(),
        );
    ...
    async fn push_on_single_page() {
        let mut storage = QueueStorage::new(
            ...
            const { RingbufferConfig::new(0x000..0x400) },
            NoCache::new(),
        );
